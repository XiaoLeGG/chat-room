...
public class MessageBox extends HBox {
	...
	private void init() {
		...
		if (this.content.getType() == ChatInformationType.FILE) {
			final String name = this.content.getContent().getString("file");
			final String remote = this.content.getContent().getString("remote-file");
			final long length = this.content.getContent().getLong("length");
			...
			if (length > 1024 * 1024) {
				...
				fileLength.setText(name + " (" + String.format("%.2f", (double) length / 1024) + "kb)");
			} else {
