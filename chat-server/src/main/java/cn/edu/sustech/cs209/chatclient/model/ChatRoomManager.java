...
public class ChatRoomManager {
  ...
  public synchronized ChatRoom createChatRoom(String title, RoomType type, String... users) {
    if (users.length < 2) {
      return null;
    }
    ...
    if (type == RoomType.PRIVATE) {
      ...
      for (ChatRoom room : rooms.values()) {
        if (room.getType() == RoomType.PRIVATE) {
          if ((users[0].equals(room.getUsers()[0]) && users[1].equals(room.getUsers()[1]))
              || (users[0].equals(room.getUsers()[1]) && users[1].equals(room.getUsers()[0]))) {
            return null;
          }
        }
      }
  ...
  public void init() {
    ...
    for (File file : dir.listFiles(e -> e.getName().toLowerCase().endsWith(".json"))) {
      try {
        ...
        ChatRoomHistory fullHistory =
            (ChatRoomHistory)
                JSON.parseObject(content.get("history").toString(), ChatRoomHistory.class);
